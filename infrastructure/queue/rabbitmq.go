...
func (r *RabbitMQ) Consume(channel chan amqp.Delivery) {
	...
	incomeMessage, err := r.Channel.Consume(
		q.Name,         // queue
	...
	go func() {
		for message := range incomeMessage {
			log.Println("Income new message")
			channel <- message
		}
...
func (r *RabbitMQ) Notify(msg, contentType, exchange, routingKey string) error {
